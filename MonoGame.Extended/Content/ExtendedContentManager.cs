...
namespace MonoGame.Extended.Content
{
    ...
    public Texture2D LoadTexture2D(string path)
    {
        ...
        public BitmapFont LoadBitmapFont(string path)
        {
            ...
            using (FileStream stream = GetStream(path))
            {
                ...
                foreach (var charBlock in bmfFile.Characters)
                {
                    ...
                    var region = new Texture2DRegion(texture, charBlock.X, charBlock.Y, charBlock.Width,
                        charBlock.Height);
                    var character = new BitmapFontCharacter((int)charBlock.ID, region, charBlock.XOffset,
                        charBlock.YOffset, charBlock.XAdvance);
                ...
                var bmfFont = new BitmapFont(bmfFile.FontName, bmfFile.Info.FontSize, bmfFile.Common.LineHeight,
                    characters.Values);
        ...
        private bool NoExtension(string name) => string.IsNullOrEmpty(System.IO.Path.GetExtension(name));
