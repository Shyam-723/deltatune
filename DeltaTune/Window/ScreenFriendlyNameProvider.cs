...
{
    // Taken from https://stackoverflow.com/questions/4958683/how-do-i-get-the-actual-monitor-name-as-seen-in-the-resolution-dialog
    public static class ScreenFriendlyNameProvider
    {
        ...
        public static IDictionary<string, string> GetAllMonitorsFriendlyNames()
        {
            IDictionary<string, string> monitorsNameMap = new Dictionary<string, string>();
            ...
            for (int i = 0; i < modeCount; i++)
            {
                ...
                {
                    target = GetMonitorFriendlyName(displayModes[i].adapterId, displayModes[i].id);
                } 
                else if (displayModes[i].infoType == DISPLAYCONFIG_MODE_INFO_TYPE.DISPLAYCONFIG_MODE_INFO_TYPE_SOURCE)
                {
                    source = GetGdiDeviceName(displayModes[i].adapterId, displayModes[i].id);
                }

                if (source != null && target != null)
                {
                    monitorsNameMap.Add(source, target);
                    source = null;
                    target = null;
                }
            }
            
            return monitorsNameMap;
        }
    }
}